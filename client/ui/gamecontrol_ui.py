...
class GameControlUI(tk.Frame):
    ...
    def __init__(self, master, my_color, match_id, user_id, client, on_back):
        ...
        self.status_label = tk.Label(
            self,
            text="Your turn" if self.my_color == "white" else "Opponent's turn",
            font=("Helvetica", 11),
            bg="#f0f0f0",
            fg="#2b2b2b",
            wraplength=220
        ...
        # ACTION BUTTONS
        # =========================
        self.btn_pause = tk.Button(self, text="Pause", width=18, command=self.pause)
        self.btn_resume = tk.Button(self, text="Resume", width=18, command=self.resume)

        self.btn_offer_draw = tk.Button(self, text="Offer Draw", width=18, command=self.offer_draw)
        self.btn_accept_draw = tk.Button(
            self, text="Accept Draw", width=18,
            bg="#7fc97f", command=self.accept_draw
        )
        self.btn_decline_draw = tk.Button(
            self, text="Decline Draw", width=18,
            bg="#f44336", command=self.decline_draw
        )

        self.btn_surrender = tk.Button(self, text="Surrender", width=18, command=self.surrender)

        self.btn_rematch = tk.Button(self, text="Rematch", width=18, command=self.rematch)
        self.btn_accept_rematch = tk.Button(
            self, text="Accept Rematch", width=18,
            bg="#7fc97f", command=self.accept_rematch
        )
        self.btn_decline_rematch = tk.Button(
            self, text="Decline Rematch", width=18,
    ...
    def update_ui_by_state(self):
        for btn in [
            self.btn_pause, self.btn_resume,
            self.btn_offer_draw, self.btn_accept_draw, self.btn_decline_draw,
            self.btn_surrender,
            self.btn_rematch, self.btn_accept_rematch, self.btn_decline_rematch
        ]:
            btn.pack_forget()
        ...
        if self.game_state == self.STATE_NORMAL:
            ...
            # chờ đối phương → không cho bấm gì thêm
            pass
    ...
    def decline_draw(self):
        ...
        # Reset UI cho chính người từ chối
        self.game_state = self.STATE_NORMAL
        self.status_label.config(text="You declined the draw", fg="orange")
        self.update_ui_by_state()
